...
from {{PYT_PKG_NAME}}.log import Log, get_pod_id, trim_time
from {{PYT_PKG_NAME}} import log
...
@pytest.fixture
def mock_now():
    with patch("{{PYT_PKG_NAME}}.log.now") as mock_now:
...
def test_log_dd_mode(capsys, mock_now, mock_env):
    logger = Log("{{PYT_PKG_NAME}}")
    logger.set_dd_mode(True)
    logger.log("INFO", "test message")
    expected_output = {
        "status": "INFO",
        "subsystem": "{{PYT_PKG_NAME}}",
        "pod_id": "0",
        "timestamp": log.now(),
...
def test_log_non_dd_mode(capsys, mock_now, mock_env):
    ...
    logger.log("INFO", "test message")
    expected_output = f"{log.now()} INFO : unknown-deployment : unknown-environment : {{PYT_PKG_NAME}} : test message\n"
